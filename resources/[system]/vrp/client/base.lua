...
CreateThread(function()
    ...
    while true do
        ...
        if DoesEntityExist(playerPed) and not IsEntityDead(playerPed) then
            ...
            if idleTime >= 3600 then -- 1800 segundos = 30 minutos
		...
		if blipsAdmin then
			...
			for k,v in pairs(userPlayers) do
				...
				if uPlayer ~= PlayerId() and NetworkIsPlayerConnected(uPlayer) then
					...
					if distance <= 1000 and ocgHtNdhlF[k] ~= nil and ocgHtNdhlF[k] ~= 1 and ocgHtNdhlF[k] ~= 2 then
...
-- 		if LoadAnim(animSequency[1]) then 
