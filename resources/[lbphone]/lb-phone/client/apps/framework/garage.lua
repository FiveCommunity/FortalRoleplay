...
local isSignalHandlerRegistered = false
...
RegisterNUICallback("Garage", function(data, cb)
    ...
    elseif action == "setWaypoint" then
        ...
        if coords then
            if not isSignalHandlerRegistered then
                isSignalHandlerRegistered = true
                RegisterNetEvent("garage:returnSignalBlocker")
                AddEventHandler("garage:returnSignalBlocker", function(hasBlocker)
                    if hasBlocker then
                        TriggerEvent("phone:sendNotification", {
                            app = "Garage",
                            title = L("Garagem"),
                            content = L("O sinal do seu veículo está bloqueado!"),
                        })
                    else
                        ...
                        TriggerEvent("phone:sendNotification", {
                            app = "Garage",
                            title = L("Garagem"),
                            content = L("Seu veículo foi marcado no seu gps"),
                        })
                    end
                end)
            end

            TriggerServerEvent("garage:hasSignalBlocker", data.plate)
        else
