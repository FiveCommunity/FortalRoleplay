...
AddEventHandler("blipsystem:serviceEnter",function(source,service,color)
    if userList[source] == nil then
        userList[source] = { service, color, "onfoot" }
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- BLIPSYSTEM:UPDATEVEHICLESTATUS (NOVO EVENTO)
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("blipsystem:updateVehicleStatus")
AddEventHandler("blipsystem:updateVehicleStatus", function(vehicleTeamStatus)
    local source = source
    if userList[source] then
        userList[source][3] = vehicleTeamStatus
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SERVICEEXIT (inalterado)
-----------------------------------------------------------------------------------------------------------------------------------------
