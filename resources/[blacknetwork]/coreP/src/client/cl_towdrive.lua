...
-- TOWDRIVER:INVOKETOW
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("towdriver:invokeTowJob")
AddEventHandler("towdriver:invokeTowJob",function()
	local ped = PlayerPedId()
	local vehicle = GetLastDrivenVehicle()
	if not serviceLocate then TriggerEvent("Notify","important","Atenção","Você não está em serviço.","vermelho",5000)  return end 
...
AddEventHandler("towdriver:invokeTow",function()
	local ped = PlayerPedId()
	local vehicle = GetLastDrivenVehicle()
	if (GetEntityModel(vehicle) == GetHashKey("flatbed") or GetEntityModel(vehicle) == GetHashKey("flatbed2") or GetEntityModel(vehicle) == GetHashKey("energyrepairnaga")) and not IsPedInAnyVehicle(ped) then
		local vehTowed = vRP.nearVehicle(10)
