...
RegisterNetEvent("tropecar:acao")
AddEventHandler("tropecar:acao", function()
...
RegisterNetEvent("evento:teleportar")
AddEventHandler("evento:teleportar", function(coords)
    local ped = PlayerPedId()
    SetEntityCoords(ped, coords.x, coords.y, coords.z, false, false, false, true)
    TriggerEvent("Notify", "verde", "Você foi teleportado para o evento!", 5000)
end)


local showPassports = false 

local function drawPassports(x,y,z,text,weight)
...
function src.passportInit()
	if showPassports or not MumbleIsConnected() then return end

	showPassports = true
	local playerList = vRPC.userPlayers()
	...
	while showPassports do
		...
		for k,v in pairs(userPlayers) do
			...
			if uPlayer ~= PlayerId() and NetworkIsPlayerConnected(uPlayer) then
				...
				if distance <= 5 then
					drawPassports(uCoords["x"],uCoords["y"],uCoords["z"] + 1.10,playerList[k],0.45)
				end
			end
		end
