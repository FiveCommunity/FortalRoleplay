...
local weaponsDamage = {
    ...
    [GetHashKey('WEAPON_CADEIRA')] = false,
...
AddEventHandler("gameEventTriggered", function(eventName, args)
    if eventName == "CEventNetworkEntityDamage" then
        ...
        if ped == PlayerPedId() then
            if weaponHash == GetHashKey("WEAPON_SMOKEGRENADE") then
                SetEntityHealth(ped, GetEntityHealth(ped) + 5)
                return
            end

            -- local _, bone = GetPedLastDamageBone(ped)
            -- local _, currentWeapon = GetCurrentPedWeapon(attacker)

            -- print(bone == 31086, weaponsDamage[currentWeapon])

            -- if IsEntityAPed(ped) and bone == 31086 and weaponsDamage[currentWeapon] then
