...
AddEventHandler("register:openSystem", function(shopId)
    if not serverAPI.applyTimers(shopId) then return end

    local ped = PlayerPedId()
    local coords = vec3(registers[shopId][1], registers[shopId][2], registers[shopId][3])

    SetEntityHeading(ped, registers[shopId][4])
    SetEntityCoords(ped, coords.x, coords.y, coords.z - 1, 1, 0, 0, 0)

	SetPedComponentVariation(PlayerPedId(), 5, 23, 1, 2)

    Wait(200)

    startCashGrabAnim()

    local safeCracking = exports["taskbar"]:taskThree()

    ClearPedTasks(ped)
    Wait(100)

    if safeCracking then
        serverAPI.paymentRegister(registers[shopId][5])
    end
end)
