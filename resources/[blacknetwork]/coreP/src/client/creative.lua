local restrictedZones = {
    { coords = vector3(924.73, -271.89, 67.9), radius = 50.0 }, 
    { coords = vector3(73.81, 121.13, 79.19), radius = 50.0 }, 
    { coords = vector3(68.37, 119.45, 79.13), radius = 100.0 }, 
}
...
AddEventHandler("entityCreated", function(entity)
    ...
    SetTimeout(0, function()
        ...
        if driver ~= 0 and driver ~= -1 and not IsPedAPlayer(driver) then
...
local weaponAdded = nil
local customAnimEnabled = false

RegisterCommand("customWeaponAnim",function()
	customAnimEnabled = not customAnimEnabled
end)

RegisterKeyMapping("customWeaponAnim","Alternar animação da arma na mão.","keyboard","M")
...
CreateThread(function()
    while true do
        ...
        if customAnimEnabled and IsPedArmed(ped, 4) then
...
local stingerModel = "p_ld_stinger_s"
local puncturedVehicles = {}

CreateThread(function()
    while true do
        Wait(50) -- Verifica a cada 1/4 de segundo

        local playerPed = PlayerPedId()
        local vehicle = GetVehiclePedIsIn(playerPed, false)
        
        if vehicle ~= 0 and not puncturedVehicles[vehicle] then
            local vehicleCoords = GetEntityCoords(vehicle)
            
            for prop in EnumerateObjects() do
                if GetEntityModel(prop) == GetHashKey(stingerModel) then
                    local propCoords = GetEntityCoords(prop)
                    
                    if #(vehicleCoords - propCoords) < 2.5 then 
                        for i = 0, 5 do 
                            SetVehicleTyreBurst(vehicle, i, true, 1000.0)
                        end
                        puncturedVehicles[vehicle] = true
                    end
                end
            end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            if GetPedInVehicleSeat(vehicle,-1) == ped then
                ...
                if speed ~= oldSpeed then
                    if oldSpeed >= 200 and (oldSpeed - speed) >= 60 then
...
local blips = {
	...
	{ 300.51,-585.07,43.29,489,1,"Hospital",0.7 },
    ...
    { 322.84,540.16,103.58,75,13,"Loja de Tatuagem",0.5 },
    { -3169.62,1075.8,20.83,75,13,"Loja de Tatuagem",0.5 },
    -- { 5464.07,3747.9,33.03,75,13,"Loja de Tatuagem",0.5 },
    { -293.57,6199.85,31.48,75,13,"Loja de Tatuagem",0.5 },
	{ 437.61,-981.73,30.68,60,54,"Departamento Policial",0.7 },
	{ 371.25,-1612.76,29.28,60,54,"Departamento Policial",0.7 },

  	{ -815.12,-544.15,37.57,71,62,"Barbearia",0.5 },
    { 138.13,-1706.46,29.3,71,62,"Barbearia",0.5 },
    { -1280.92,-1117.07,7.0,71,62,"Barbearia",0.5 },
    { 1930.54,3732.06,32.85,71,62,"Barbearia",0.5 },
    { 1214.2,-473.54,66.21,71,62,"Barbearia",0.5 },
...
--[[ 	{ -905.92,-2045.05,9.3,60,54,"Departamento Policial",0.7 },
	...
	{161.06,-3028.82,6.77,402,1,"Mecânica Tunners",0.7},
	{889.94,-2114.05,30.46,402,1,"Mecânica Raijin",0.7},
	...
	{ -71.34,59.65,71.91,439,73,"Concessionária Vip",0.7 }, 
	...
	{ 681.72,-1631.05,12.72,407,0,"Ponto Turistico",0.3 }, 
	{ -2039.96,-532.35,8.53,407,0,"Ponto Turistico",0.3 }, 
	{ -1048.04, -472.16, 36.61, 310, 0, "Arena", 0.7 },
    {-1423.23,-2658.94,13.93,293,0,"Studio",0.9}
...
Citizen.CreateThread(function()
    for _,v in pairs(blips) do
        ...
        AddTextComponentString(v[6] or "Blip") 
...
Citizen.CreateThread(function()
	while true do
		...
		SetWeaponDamageModifierThisFrame("WEAPON_CADEIRA",0.25)
...
--[ PLACAS ]-----------------------------------------------------------------------------------------------------------------------------
imageUrl = "https://r2.fivemanage.com/n0xgOPJavUVcv2vqPq8jt/images/mercosul.png" -- 
...
local noPedZones = {
    {coords = vector3(-943.05,-2051.56,18.21), radius = 100.0}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for i, zone in ipairs(noPedZones) do
            ClearAreaOfPeds(zone.coords, zone.radius, true)
            ClearAreaOfVehicles(zone.coords, zone.radius, false)
            RemoveVehiclesFromGeneratorsInArea(
                zone.coords.x - zone.radius, zone.coords.y - zone.radius, zone.coords.z - 10.0,
                zone.coords.x + zone.radius, zone.coords.y + zone.radius, zone.coords.z + 10.0
            )
            
            SuppressShockingEventsNextFrame()

            if not blockingAreas[i] then
                blockingAreas[i] = AddScenarioBlockingArea(
                    zone.coords.x - zone.radius, zone.coords.y - zone.radius, zone.coords.z - 10.0,
                    zone.coords.x + zone.radius, zone.coords.y + zone.radius, zone.coords.z + 10.0,
