...
```lua
local coreP_cooldown = exports['coreP']
local success, message = coreP_cooldown:register('player', 10)
if not success then
...
```lua
local coreP_cooldown = exports['coreP']
local success, message = coreP_cooldown:deleteCooldown('player')
if not success then
  return print(message)
end
```
