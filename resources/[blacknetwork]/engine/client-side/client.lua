...
-----------------------------------------------------------------------------------------------------------------------------------------
local fuelLocs = {
	{ 613.00,273.94,103.28,2.0,0.075 },
	{ 613.00,263.70,103.28,2.0,0.075 },
	{ 621.48,263.72,103.28,2.0,0.075 },
	{ 621.51,273.97,103.28,2.0,0.075 },
	{ 629.02,273.95,103.28,2.0,0.075 },
	{ 629.05,263.89,103.28,2.0,0.075 },
	...
	{ -722.8,-1471.93,5.0,229.61,0.075 },
	{ -701.06,-1447.19,5.0,232.45,0.075 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- FLOOR
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if GetVehicleFuelLevel(vehicle) >= 1 then
				if speed >= 1 then
					...
					if vehFuels[vehPlate] ~= nil then
						...
						local consumoMultiplicador = 3.4
						if carrosBlu[modelName] then
							consumoMultiplicador = 0.05
						end
						vehFuels[vehPlate] = vehFuels[vehPlate] - ((vehFuel[floor(GetVehicleCurrentRpm(vehicle))] or 1.0) * (vehClass[vehClasses] or 1.0) * consumoMultiplicador / 10)
...
AddEventHandler("engine:vehTuning",function()
	...
	if vehicle then
		...
		"Motor: Nível " .. motor .. " / 4<br>" ..
		"Freio: Nível " .. freio .. " / 3<br>" ..
		"Transmissão: Nível " .. transmissao .. " / 3<br>" ..
		"Suspensão: Nível " .. suspensao .. " / 4<br>" ..
		"Blindagem: Nível " .. blindagem .. " / 5<br>" ..
		"Lataria: " .. parseInt(body / 10) .. "%<br>" ..
		"Nitro: " .. parseInt(GlobalState["Nitro"][plate] or 0) .. "%<br>" ..
		"Motor: " .. parseInt(engine / 10) .. "%<br>" ..
		"Gasolina: " .. parseInt(fuel) .. "%", 
	"amarelo", 10000)
...
RegisterNetEvent("engine:fillFuel")
AddEventHandler("engine:fillFuel", function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        ...
        vehFuels[plate] = 100.0
        TriggerServerEvent("engine:tryFuel", plate, 100.0)

        TriggerEvent("Notify", "sucesso", "Tanque abastecido para 100%.", "verde", 5000)
    else
        TriggerEvent("Notify", "negado", "Você precisa estar dentro de um veículo.", "vermelho", 5000)
    end
end)
