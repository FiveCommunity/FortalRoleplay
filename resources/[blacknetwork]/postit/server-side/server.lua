...
RegisterCommand("delpostit", function(source, args)
    local user_id = vRP.getUserId(source)
    if not user_id then return end

    local globalPostIts = GlobalState["Postit"]
    if not globalPostIts then return end
    ...
    for id, postit in pairs(globalPostIts) do
        local x, y, z = postit[1], postit[2], postit[3]
    ...
    if closestId then
        local postit = globalPostIts[closestId]
        local donoId = postit[6]

        if user_id == donoId and os.time() <= postit[7] then
            vRP.generateItem(user_id, "postit", 1, true)
        end

        globalPostIts[closestId] = nil
        GlobalState["Postit"] = globalPostIts

        TriggerClientEvent("postit:deletePostIts", -1, closestId)
        TriggerClientEvent("Notify", source, "verde", "Post-It removido com sucesso.", 5000)
    else
        TriggerClientEvent("Notify", source, "amarelo", "Nenhum Post-It próximo encontrado.", 5000)
    end
end)
