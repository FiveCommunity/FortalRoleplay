-----------------------------------------------------------------------------------------------------------------------------------------
-- VRP
-----------------------------------------------------------------------------------------------------------------------------------------
local Tunnel = module("vrp","lib/Tunnel")
-----------------------------------------------------------------------------------------------------------------------------------------
-- CONNECTION
-----------------------------------------------------------------------------------------------------------------------------------------
cRP = {}
Tunnel.bindInterface("robberys",cRP)
vSERVER = Tunnel.getInterface("robberys")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local robberys = {}
local robberyId = "1"
local robberyTimer = 0
local robberySeconds = 0
local robberyActive = false
local activeTimers = GetGameTimer()

-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
	while true do
		local timeDistance = 999
		local ped = PlayerPedId()
		if not IsPedInAnyVehicle(ped) then
			local coords = GetEntityCoords(ped)
			
			if not robberyActive then
				
				for k,v in pairs(robberys) do
					local distance = #(coords - vector3(v["coords"][1],v["coords"][2],v["coords"][3]))

					if distance <= 1 then
						timeDistance = 1
						
						if IsControlJustPressed(1,38) and robberySeconds <= 0 then
							robberySeconds = 5
						
							if vSERVER.checkRobbery(k) then
								robberyId = k
								robberyTimer = v["timer"]
								activeTimers = GetGameTimer()
								robberyActive = true
						
								local ped = PlayerPedId()
								SetEntityHeading(ped, v["coords"][4] or GetEntityHeading(ped))
								SetEntityCoords(ped, v["coords"][1], v["coords"][2], v["coords"][3] - 1, false, false, false, false)
						
								RequestAnimDict("anim@heists@ornate_bank@grab_cash")
								while not HasAnimDictLoaded("anim@heists@ornate_bank@grab_cash") do
									Wait(10)
								end

								TaskPlayAnim(ped, "anim@heists@ornate_bank@grab_cash", "grab", 8.0, 8.0, -1, 49, 0, false, false, false)

								local ped = PlayerPedId()
								local drawable = GetPedDrawableVariation(ped, 5)
								local texture = GetPedTextureVariation(ped, 5)

								if drawable ~= 22 or texture ~= 1 then
									SetPedComponentVariation(ped, 5, 23, 1, 2)
								end


								SendNUIMessage({ show = true, timer = "AGUARDE "..robberyTimer.." SEGUNDOS" })

								Citizen.CreateThread(function()
									Wait(15000)
									ClearPedTasks(ped)
								end)
							end
						end
						
					end
				end
			else
				local distance = #(coords - vector3(robberys[robberyId]["coords"][1],robberys[robberyId]["coords"][2],robberys[robberyId]["coords"][3]))
				if distance > robberys[robberyId]["distance"] or GetEntityHealth(ped) <= 101 then
					SendNUIMessage({ show = false })
					robberyActive = false
				end
			end
		end

		Citizen.Wait(timeDistance)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
	while true do
		if robberyActive then
			if GetGameTimer() >= activeTimers then
				robberyTimer = robberyTimer - 1
				activeTimers = GetGameTimer() + 1000
				SendNUIMessage({ timer = "AGUARDE "..robberyTimer.." SEGUNDOS" })

				if robberyTimer <= 0 then
					vSERVER.paymentRobbery(robberyId)
					SendNUIMessage({ show = false })
					robberyActive = false
				end
			end
		end

		Citizen.Wait(1000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSECONDS
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
	while true do
		if robberySeconds > 0 then
			robberySeconds = robberySeconds - 1
		end

		Citizen.Wait(1000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INPUTROBBERYS
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.inputRobberys(robberyTables)

	robberys = robberyTables

	local innerTable = {}
	for k,v in pairs(robberys) do
		table.insert(innerTable,{ v["coords"][1],v["coords"][2],v["coords"][3],1,"E",v["name"],"Pressione para iniciar o roubo" })
	end

	TriggerEvent("hoverfy:insertTable",innerTable)
end

local Proxy = module("vrp","lib/Proxy")
vRP = Proxy.getInterface("vRP")

local atmList = {
	{ 147.82,-1035.85,29.34,164.41 },
	{ 146.06,-1035.12,29.34,155.91 },
	{ 111.3,-775.31,31.44,351.5 },
	{ 114.37,-776.41,31.41,345.83 },
	{ 114.4,-776.42,31.41,343.0 },
	{ 112.75,-819.4,31.34,161.58 },
	{ 118.3,1951.87,14.67,279.75 },
	{ 295.72,-896.09,29.22,274.97 },
	{ 296.4,-894.03,29.23,257.96 },
	{ 50.85,364.48,15.41,185.7 }, 
	{ 33.31,-1348.18,29.49,184.26 },
	{-97.34,6455.44,31.46,42.52},
	{-95.49,6457.26,31.46,42.52},
	{-133.01,6366.48,31.48,317.49},
	{-283.13,6226.11,31.49,320.32},
	{174.19,6637.85,31.58,42.52},
	{155.86,6642.78,31.59,306.15},
	{-386.98,6046.2,31.49,320.32},
	{1701.2,6426.45,32.76,62.37},
	{1735.22,6410.64,35.03,153.08},
	{2682.98,3286.52,55.23,238.12},
	{1686.85,4815.78,42.01,283.47},
	{1822.7,3683.06,34.27,201.26},
	{1968.06,3743.58,32.33,209.77},
	{540.27,2671.07,42.16,0.0},
	{1172.58,2702.5,38.16,5.67},
	{1171.53,2702.48,38.16,357.17},
	{2564.66,2584.63,38.08,113.39},
	{2558.44,389.46,108.61,272.13},
	{2558.76,350.96,108.61,85.04},
	{1077.92,-776.45,58.23,184.26},
	{1153.75,-326.8,69.2,104.89},
	{1167.04,-456.11,66.79,348.67},
	{285.47,143.4,104.17,155.91},
	{380.73,323.44,103.56,161.58},
	{228.14,338.39,105.56,161.58},
	{-165.07,232.71,94.91,79.38},
	{-165.08,234.76,94.91,85.04},
	{-1430.07,-211.1,46.51,119.06},
	{-1415.96,-212.0,46.51,232.45},
	{-2072.42,-317.18,13.31,269.3},
	{-1386.43,-596.29,30.31,291.97},
	{-1386.3,-597.23,30.31,266.46},
	{-1386.65,-598.18,30.31,229.61},
	{-1200.75,-885.4,13.26,218.27},
	{-1314.75,-836.01,16.95,308.98},
	{-1315.8,-834.71,16.95,308.98},
	{-821.69,-1081.99,11.12,31.19},
	{-717.69,-915.67,19.21,93.55},
	{-809.34,-1238.21,7.33,51.03},
	{-537.81,-854.47,29.28,187.09},
	{-526.64,-1222.95,18.45,158.75},
	{-219.04,-1311.79,31.29,178.59},
	{130.0,-1292.66,29.27,300.48},
	{5.2,-919.79,29.55,246.62},
	{129.67,-1291.88,29.27,297.64},
	{24.47,-946.04,29.35,340.16},
	{129.18,-1291.17,29.27,303.31},
	{289.01,-1256.79,29.44,272.13},
	{119.12,-883.72,31.12,76.54},
	{288.77,-1282.37,29.64,266.46},
	{112.7,-819.29,31.34,167.25},
	{111.33,-775.33,31.44,343.0},
	{114.52,-776.42,31.41,348.67},
	{314.11,-279.11,54.17,161.58},
	{-254.25,-692.51,33.62,150.24},
	{-256.18,-716.14,33.51,68.04},
	{-258.92,-723.5,33.48,70.87},
	{-350.99,-49.89,49.03,158.75},
	{-721.06,-415.46,34.98,260.79},
	{-1205.01,-326.28,37.83,116.23},
	{-1205.64,-324.84,37.86,113.39},
	{-303.37,-829.72,32.42,354.34},
	{-301.67,-830.05,32.42,354.34},
	{-203.96,-861.44,30.26,22.68}
}

RegisterNetEvent("eletronics:stopRobbery",function()
	inService = false 
	inTimers = 0 
end) 

AddEventHandler("eletronics:openSystem", function(shopId)
	if vSERVER.checkSystems(shopId) then
		currentTimer = GetGameTimer()
		inTimers = 18
		inService = true
		TriggerEvent("setActive",18)
		TriggerEvent("Progress",18000)
		LocalPlayer["state"]["Cancel"] = true
		LocalPlayer["state"]["Commands"] = true

		local ped = PlayerPedId()
		SetEntityHeading(ped, atmList[shopId][4])
		SetEntityCoords(ped, atmList[shopId][1], atmList[shopId][2], atmList[shopId][3] - 1, 1, 0, 0, 0)

		RequestAnimDict("anim@heists@ornate_bank@grab_cash")
		while not HasAnimDictLoaded("anim@heists@ornate_bank@grab_cash") do
			Wait(10)
		end
		TaskPlayAnim(ped, "anim@heists@ornate_bank@grab_cash", "grab", 8.0, 8.0, -1, 49, 0, false, false, false)

		local ped = PlayerPedId()
		local drawable = GetPedDrawableVariation(ped, 5)
		local texture = GetPedTextureVariation(ped, 5)

		if drawable ~= 22 or texture ~= 1 then
			SetPedComponentVariation(ped, 5, 23, 1, 2)
		end


		Citizen.CreateThread(function()
			Wait(15000)
			ClearPedTasks(ped)
		end)

		while inService do
			if inTimers > 0 and GetGameTimer() >= currentTimer then
				inTimers = inTimers - 1
				currentTimer = GetGameTimer() + 1000

				if inTimers <= 0 then
					vSERVER.paymentAtm()
					LocalPlayer["state"]["Commands"] = false
					LocalPlayer["state"]["Cancel"] = false
					vRP.removeObjects()
					inService = false
					break
				end
			end
			Citizen.Wait(1)
		end
	end
end)


AddEventHandler('onClientResourceStart', function (resourceName)
	if(GetCurrentResourceName() ~= resourceName) then
	  return
	end
	vSERVER.ResourceStarted()
end)