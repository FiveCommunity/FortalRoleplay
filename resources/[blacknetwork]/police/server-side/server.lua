	...
	if user_id and vRP.getHealth(source) > 100 then
		if vRP.hasGroup(user_id,"Police") then
			...
			if mHash == "mp_m_freemode_01" or mHash == "mp_f_freemode_01" then
				TriggerEvent("fortal-character:Server:Skinshop:Save", entity[1],preset[mHash])
			end
		end
	end
end)
...
-- REDUCEPRISON
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("prisonclear", function(source, args, rawCommand)
    local adminSource = source
    local user_id = vRP.getUserId(adminSource)
    
    if vRP.hasPermission(user_id, "Admin") then
        local target_id = tonumber(args[1])
        if target_id then
            vRP.updatePrison(target_id, 0) 
            local targetSource = vRP.getUserSource(target_id)
            
            if targetSource then
                vCLIENT.syncPrison(targetSource, false, true)
                TriggerClientEvent("Notify", targetSource, "verde", "Você foi liberado da prisão", 5000)
                TriggerClientEvent("Notify", adminSource, "verde", "Jogador liberado da prisão com sucesso.", 5000)
            else
                TriggerClientEvent("Notify", adminSource, "amarelo", "Jogador está offline. Prisão zerada no banco.", 5000)
            end
        else
            TriggerClientEvent("Notify", adminSource, "vermelho", "Uso: /prisonclear [user_id]", 5000)
        end
    else
        TriggerClientEvent("Notify", adminSource, "vermelho", "Você não tem permissão para isso.", 5000)
    end
end)
