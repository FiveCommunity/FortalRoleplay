  ...
  return (
    <nav className="flex flex-col items-center">
      <div className="flex flex-col gap-1.5">
        ...
        {Filters[page].map((id: string) => {
          const active = (): boolean => {
            return filter.current === id;
          };
          const select = () => {
            filter.set(id);
          };
          return (
            ...
            >
              ...
              >
                {Icons.Filter[id.charAt(0).toUpperCase() + id.slice(1).toLowerCase()] && createElement(Icons.Filter[id.charAt(0).toUpperCase() + id.slice(1).toLowerCase()])}
              </div>
            </button>
          );
        })}
