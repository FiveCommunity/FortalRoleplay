      ...
      <button
        onClick={() => {
          setPopup({
            ...
            description: `Tem certeza que deseja salvar sua aparência por $${price}?`,
            callback: () => Post.create("Barbershop:SavePed", {
              price,
            })
          })          
        }}
        disabled={price < 1}
        className={cn(
          "bg-gradient-primary flex h-[2.875rem] w-full items-center justify-between rounded px-4",
          price >= 1 && "hover:opacity-80 active:scale-95 transition-all duration-200",
        )}
      >
...
function Rotate() {
  ...
  return (
    <div className="bg-section flex h-[2.5625rem] w-full items-center rounded border border-white/5 pr-[1.13rem]">
      <p className="grid flex-none w-20 h-full font-bold leading-none place-items-center text-white/90">
        {direction}°
      </p>
      <Slider
