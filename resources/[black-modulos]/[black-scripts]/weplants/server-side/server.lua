...
function cnVRP.checkTimers(id)
	...
	if user_id then
		if wePlants[id] ~= nil and parseInt(wePlants[id][4]) >= 100 then
			-- local myBackpack = 0
			-- if vRP.getBackpack(user_id) == 20 then
			-- 	myBackpack = 20
			-- elseif vRP.getBackpack(user_id) == 25 then
			-- 	myBackpack = 25
			-- elseif	vRP.getBackpack(user_id) == 35 then
			-- 	myBackpack = 35
			-- elseif vRP.getBackpack(user_id) == 50 then
			-- 	myBackpack = 50
			-- elseif 	vRP.getBackpack(user_id) == 75 then
			-- 	myBackpack = 75
			-- elseif vRP.getBackpack(user_id) == 100 then 
			-- 	myBackpack = 100	
			-- end
			
			if (parseInt(vRP.inventoryWeight(user_id)) + 3) <= vRP.getWeight(user_id) then
				...
				vRP.giveInventoryItem(user_id,"weed",parseInt(mit),1)
				...
				TriggerClientEvent("Notify",source,'amarelo',"Aviso","Sua <b>Mochila</b> Cheia.",5000)
			end
		end
		return false
	end
