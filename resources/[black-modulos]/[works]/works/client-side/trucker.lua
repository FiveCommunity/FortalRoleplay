			...
			if getPackage then
				...
				if distance <= 50 then
					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 and distance <= 5 then
						...
						local vehicle,vehNet,vehPlate,modelVehicle = vRP.vehList(15)
				...
				if distance <= 50 then
					...
					if GetEntityModel(veh) == 569305213 then
						...
						if IsControlJustPressed(1,38) and inSeconds <= 0 and not isTake then
							...
							vGARAGES.serverVehicle(deliveryCoords[deliveryLocates][4],vehCoords["x"],vehCoords["y"],groundZ, GetEntityHeading(ped), vRP.generatePlate(), 0, nil, 1000)
							isTake = true
						end
					end
				end
