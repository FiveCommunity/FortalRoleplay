...
Config.useModernUI = true -- Em março de 2023 os trabalhos passaram por uma grande reformulação, e a UI foi alterada. Defina como false para usar a UI ANTIGA (não mais suportada).
Config.splitReward = false -- Esta opção funciona apenas quando useModernUI é false. Se esta opção for true, o pagamento será: (Config.OnePercentWorth * Progresso) / ContagemDoGrupo, se for false: (Config.OnePercentWorth * Progresso)
Config.UseBuiltInNotifications = true   -- Defina como false se quiser usar o estilo de notificação do seu framework. Caso contrário, as notificações modernas internas serão usadas.
Config.letBossSplitReward = true                    -- Se true, o chefe poderá gerenciar a porcentagem de recompensa do grupo no menu. Se definido como false, todos receberão o mesmo valor.
Config.multiplyRewardWhileWorkingInGroup = true     -- Se false, a recompensa permanecerá padrão. Por exemplo, $1000 por completar o trabalho. Se definido como true, o pagamento dependerá de quantos jogadores há no grupo. Por exemplo, se para um trabalho completo há $1000, então se o jogador estiver em um grupo com 4 membros, a recompensa será $4000. (recompensaBase * contagemDoGrupo)

Config.UseTarget = false      -- Altere para true se quiser usar um sistema de target. Todas as configurações sobre o sistema estão no arquivo target.lua.
Config.RequiredJob = "none"   -- Defina como "none" se não quiser usar empregos. Se estiver usando target, você precisa definir o parâmetro "job" dentro de cada exportação em target.lua.
Config.RequireJobAlsoForFriends = true          -- Se false, apenas o anfitrião precisa ter o emprego. Se true, todos no grupo precisam ter o Config.RequiredJob.
Config.RequireOneFriendMinimum = false                              -- Defina como true se quiser forçar os jogadores a criarem equipes.
Config.JobVehicleModel = "17mov_DeliveryCar"                        -- Modelo do veículo do trabalho.
Config.VehicleBackBone = "handle_dside_r"                           -- Usado apenas quando Config.Target == false. Nas coordenadas deste osso, o texto 3D para interagir com o veículo será exibido.
Config.VehicleBootOffset = 5  -- Significa 5 metros atrás das coordenadas básicas do veículo. Usado ao tentar pegar um pacote do veículo.
Config.VehicleSearchingScenario = "prop_human_parking_meter"        -- Cenário usado ao "procurar" dentro do veículo.
Config.Price = 800            -- 100$ por cada entrega.
Config.BlockHostFromWorking = true  -- Significa que se os jogadores estiverem em grupo, apenas o anfitrião pode dirigir o veículo e o parceiro precisa entregar. Defina como false se quiser permitir que o anfitrião também faça entregas.
Config.EnableVehicleTeleporting = true                              -- Se true, o script teletransportará o anfitrião para o veículo da empresa. Se false, o veículo aparecerá e todos precisarão entrar manualmente.
Config.enableSpawningPedsInDoors = true                             -- Defina como false se não quiser usar peds nas portas.
Config.JobCooldown = 0 * 60 -- 10 * 60            -- 0 minutos de cooldown entre trabalhos (no exemplo acima está configurado para 10 minutos).
Config.GiveKeysToAllLobby = true    -- Defina como false se quiser dar as chaves apenas para o criador do grupo ao iniciar o trabalho.
Config.ProgressBarOffset = "25px"                   -- Valor em px do deslocamento do contador na tela.
Config.ProgressBarAlign = "bottom-center"            -- Alinhamento da barra de progresso.
...
Config.RequireWorkClothes = false                   -- Defina como true para mudar as roupas dos jogadores toda vez que começarem o trabalho.

Config.PenaltyAmount = 500                      -- Multa aplicada quando um jogador termina o trabalho sem o veículo da empresa.
Config.DontPayRewardWithoutVehicle = false      -- Defina como true para não pagar recompensa para jogadores que quiserem terminar sem o veículo da empresa (aceitando a multa).
Config.DeleteVehicleWithPenalty = false         -- Deletar veículo mesmo se não for o veículo da empresa.

Config.RequiredItem = "none"                        -- Defina como qualquer item que quiser para exigir que os jogadores tenham ele no inventário antes de começarem o trabalho.
Config.RequireItemFromWholeTeam = true              -- Se false, apenas o anfitrião precisa ter o item necessário, caso contrário toda a equipe precisará tê-lo.

Config.RestrictBlipToRequiredJob = false            -- Defina como true para ocultar o blip do trabalho para jogadores que não têm o emprego requerido. Se o emprego requerido for "none", esta opção não terá efeito.
Config.Blips = {              -- Aqui você pode configurar o blip da empresa.
    [1] = {
        Sprite = 568,
        Color = 48,
        Scale = 0.8,
...
Config.Locations = {         -- Aqui você pode mudar todos os locais da base de trabalho. 
    DutyToggle = {
        Coords = {
...
Config.SpawnPoint = vector4(62.18,124.79,79.19,153.08)  -- Vehicle spawn point
...
Config.Lang = {
    ...
    -- Aqui você pode mudar todas as traduções usadas no client.lua e server.lua. Não se esqueça de traduzi-las também nos arquivos HTML e JS.

    -- Cliente
    ["no_permission"] = "Apenas o dono da equipe pode fazer isso!",
    ["keybind"] = 'Interação com o marcador',
    ...
    ["missingParcel"] = "Você não tem um pacote com você. Pegue-o do carro",
    ...
    ["cantInvite"] = "Para poder convidar mais pessoas, você precisa primeiro terminar o trabalho",
    ...
    ["tutorial"] = "O trabalho envolve entregar pacotes nas casas. Se você trabalhar em equipe, uma pessoa dirige o carro até o local designado, enquanto a outra pessoa pega o pacote do porta-malas e vai entregar o pacote na casa. Por outro lado, se você trabalhar sozinho, precisará fazer as duas tarefas. O pagamento depende do número de pacotes entregues. Termine o trabalho quando quiser retornando à base e deixando o carro no local designado.",
    ["partyIsFull"] = "Falha ao enviar o convite, sua equipe está cheia",
    ["inviteSent"] = "Convite enviado!",
    ["tooFar"] = "Você está muito longe do local de destino para pegar um pacote",
    ...
    ["error"] = "Houve um problema ao entrar na equipe. Tente novamente mais tarde.",
    ...
    ["clientsPenalty"] = "O host da equipe aceitou a penalidade. Você não recebeu o pagamento",
