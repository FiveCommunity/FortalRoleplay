...
function updateUserHomes(source, user_id)
	local query = vRP.query("propertys/Exist", { user_id = user_id })
	local userHomes = {}
	for k, v in pairs(query) do
		userHomes[v["name"]] = true
	end
	TriggerClientEvent("propertys:user", source, userHomes)
end
...
CreateThread(function()
	...
	TriggerClientEvent("homes:receiveInformations", -1, Informations)
				...
				if os.time() > v["tax"] then
					if vRP.request(source, "Hipoteca atrasada, deseja efetuar o pagamento?") then
						if vRP.paymentFull(getUserId, Informations[v["interior"]]["Price"] * 0.1) then
							vRP.execute("propertys/tax", { name = v["name"] })
...
AddEventHandler("propertys:Toggle", function(coords, bucket, Name)
	local source = source
	local getUserId = vRP.getUserId(source)
	...
	if getUserId then
		if not Inside[getUserId] then
			...
			current = Name
			args = { ["1"] = current }
			exports["logsystem"]:SendLog("HomeEnter", getUserId, nil, args)
		else
			Inside[getUserId] = nil
			enterCoords[source] = nil
			TriggerEvent("vRP:BucketServer", source, 0)
			args = { ["1"] = current }
			exports["logsystem"]:SendLog("HomeExit", getUserId, nil, args)
			current = nil
		end
	end
end)

RegisterNetEvent("propertys:robberyLog")
AddEventHandler("propertys:robberyLog", function(houseId)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local user_id = vRP.getUserId(source)
		args = { ["1"] = houseId }
		exports["logsystem"]:SendLog("HomeRouberysEnter", user_id, nil, args)
	end
end)
	...
	if getUserId then
		...
		if Consult == nil then
			...
			if vRP.request(source, "Deseja comprar a propriedade?") then
				if IsMansion then
					if vRP.paymentFull(getUserId, Informations[Interior]["Price"] * Mansions[Name].multiplier) then
						vRP.execute("propertys/Buy", {
							name = Name,
							interior = Interior,
							user_id = getUserId,
							price = Informations[Interior]["Price"] * Mansions[Name].multiplier,
						...
						if #Query > 0 then
							...
							TriggerClientEvent("Notify", source, "important", "Atenção",
								"Voce acabou de perder seu apartamento.", "amarelo", 3000)
						end
	
						TriggerEvent("fx:updateHomes")
						updateUserHomes(source, getUserId)
	
						args = { ["1"] = Name }
						exports["logsystem"]:SendLog("HomeBuy", getUserId, nil, args)
					else
						TriggerClientEvent("Notify", source, "cancel", "Negado", "<b>Dólares</b> insuficientes.", "vermelho",
							5000)
					end
				else
...
AddEventHandler("propertys:enterHomeUser", function(interior)
	...
	if (Lock[interior] == nil) then
		...
		args = { ["1"] = interior }
		exports["logsystem"]:SendLog("HomeEnter", user_id, nil, args)
	end
end)
		...
		if Consult[1] then
			if (parseInt(Consult[1]["user_id"]) == getUserId) then
				if Lock[Name] then
					...
					TriggerClientEvent("Notify", source, "important", "Atenção", "Propriedade destrancada.", "amarelo",
						5000)
					Lock[Name] = true
				end
			end
		...
		if getUserId then
			...
			if Consult[1] then
				if parseInt(Consult[1]["user_id"]) == getUserId then
					...
					if vRP.request(source, "Deseja vender a propriedade? por R$" .. Informations[Consult[1]["interior"]]["SellPrice"]) then
						...
						vRP.addBank(getUserId, Informations[Consult[1]["interior"]]["SellPrice"])
						TriggerEvent("fx:updateHomes")
						updateUserHomes(source, user_id)
						args = { ["1"] = Name }
						exports["logsystem"]:SendLog("HomeSell", getUserId, nil, args)
					end
				end
			end
		end
...
function Creative.OpenChest(Name, Mode)
	...
	if getUserId then
		...
		if not Owner[1] then
			TriggerClientEvent("Notify", source, "amarelo", "Erro: Propriedade não encontrada ou não pertence ao jogador.", 5000)
			return
		end
		...
		for k, v in pairs(Inv) do
			...
			v["url"] = Config.imagesProvider .. itemIndex(v["item"]) .. ".png"
			v["type"] = "item"
...
function Creative.getUserProfile()
	local source = source
	local user_id = vRP.getUserId(source)
...
function Creative.Store(Item, Slot, Amount, Target, Name, Mode)
	...
	if getUserId then
		...
		if Mode == "fridge" and not noStore[Item] then return end
			...
			if Item == "diagram" then
				...
				if vRP.storeChest(getUserId, Mode .. ":" .. Name .. "-" .. Owner[1]['user_id'], Amount, Consult[1][Mode], Slot, Target) then
					if Mode ~= "fridge" then
						...
						exports["logsystem"]:SendLog("HomeTakeChest", getUserId, getUserId, args)
					else
						args = {
							["1"] = Name,
							["2"] = Item,
						...
						exports["logsystem"]:SendLog("HomeTakeRefrigerator", getUserId, getUserId, args)
					end
				else
...
AddEventHandler("playerConnect", function(user_id, source)
	TriggerClientEvent("homes:receiveInformations", source, Informations)
					...
					if vRP.request(source, "Adicionar uma garagem por <b>$50.000</b> dólares?") then
						...
						if userPermissions[1] then
							if vRP.paymentFull(user_id, 50000) then
								TriggerClientEvent("Notify", source, "important", "Atenção",
									"Fique no local onde vai abrir a garagem e pressione a tecla <b>E</b>.", "amarelo",
									10000)
		...
		if (vRP.inventoryWeight(user_id) + (itemWeight(mobileTheft[mobile][randItem]["item"]) * parseInt(value))) <= vRP.getWeight(user_id) then
			if parseInt(#policeResult) >= 0 then
				if math.random(0, 100) <= 40 then
					...
					args = {
						["2"] = mobileTheft[mobile][randItem]["item"],
						["3"] = value
					}
					exports["logsystem"]:SendLog("HomeRouberysPickUp", user_id, nil, args)
			...
			for k, v in pairs(policeResult) do
				async(function()
					TriggerClientEvent("NotifyPush", v,
						{ code = 90, title = "Roubo de Propriedade", x = coords["x"], y = coords["y"], z = coords["z"], criminal =
						"Alarme de segurança", time = "Recebido às " .. os.date("%H:%M"), blipColor = 43 })
				end)
			end
		end
	end
...
function Creative.callPolice(coords)
	...
	if user_id then
		...
		for k, v in pairs(policeResult) do
			async(function()
				TriggerClientEvent("NotifyPush", v,
					{ code = 90, title = "Roubo de Propriedade", x = coords[1], y = coords[2], z = coords[3], criminal =
					"Alarme de segurança", time = "Recebido às " .. os.date("%H:%M"), blipColor = 43 })
			end)
		end
	end
...
function Creative.checkHotel()
	...
	if user_id then
		...
		if #Consult > 1 then
			TriggerClientEvent("Notify", source, "important", "Atenção",
				"Sua corretora passou aqui para pegar seus imoveis para sua nova casa", "amarelo", 15000)
			return false
		end
		...
		if Query[1] == nil then
			...
			TriggerClientEvent("Notify", source, "check", "Parabens", "Voce acabou de alugar um apartamento.", "verde",
				3000)
...
AddEventHandler("playerDisconnect", function(user_id, source)
	if (enterCoords[source]) then
		...
		vRP.execute("playerdata/setUserdata",
			{ user_id = user_id, key = "Position", value = json.encode({ x = mathLegth(enterCoords[source]["x"]), y =
			mathLegth(enterCoords[source]["y"]), z = mathLegth(enterCoords[source]["z"]) }) })
	end
