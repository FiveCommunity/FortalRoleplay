$(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (item !== undefined && item.type === 'IDENTITY') {
            if (item.payload === true) {
                ...
                $.post('http://black_identity/getAvatarPlayer', JSON.stringify({}), (data) => {
                ...
                $.post('http://black_identity/getInfos', JSON.stringify({}), (data) => {
                    $infosTexts.html(`
                        <div class="infos">
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            $.post('http://black_identity/exit', JSON.stringify({}));
    ...
    $(document).on("click", "#button-confirm", () => {
        ...
        if (/(.*\.png$|.*\.jpg$|.*\.jpeg$)/i.test(newLink) || newLink.indexOf('discord') !== -1) {
            ...
            $.post('http://black_identity/changeAvatar', JSON.stringify({ newLink }), (data) => {});
            $.post('http://black_identity/exit', JSON.stringify({}));
        } else {
